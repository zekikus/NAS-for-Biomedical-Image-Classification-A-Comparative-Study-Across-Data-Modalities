...
class Model(nn.Module):
    ...
    def __init__(self, chromosome = None, config = None, nbr_cell = None, nbr_stack = None, nbr_filters = None, num_classes = None, C_in = 1):
        ...
        self.org_matrix = self.create_matrix(0, self.MAX_EDGE_NBR)
        self.org_ops = self.create_matrix(self.MAX_EDGE_NBR + 3, self.MAX_EDGE_NBR * 2 + 3)   
    ...
    def create_matrix(self, start_idx, end_idx):
        ...
        self.matrix[triu_indices] = self.config[start_idx : end_idx]
            ...
            for epoch in range(120):
                ...
                for inputs, labels in train_loader:
                    ...
                    with torch.set_grad_enabled(True):
                        ...
                        error = loss_fn(output.float(), labels.squeeze(1))
                        train_loss.append(error.item())
                        train_acc.append(metric_fn(output.to('cpu'), labels.squeeze(1).to('cpu')).item())
                        optimizer.zero_grad()
                        error.backward()
                        optimizer.step()
                ...
                with torch.no_grad():
                    for inputs, labels in val_loader:
                        ...
                        inputs, labels = inputs.to(device), labels.to(device)
                        output = self.forward(inputs)
                        error = loss_fn(output, labels.squeeze(1))
                        val_dice.append(metric_fn(output.to('cpu'), labels.squeeze(1).to('cpu')).item())
    ...
    def prune(self, original_matrix, original_ops):
        ...
        ops = matrix * ops # remove zero edges
        ...
        ops = np.delete(ops, list(extraneous), axis=0)
        ops = np.delete(ops, list(extraneous), axis=1)
