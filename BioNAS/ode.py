...
class DE():
    ...
    def __init__(self, pop_size = None, 
        ...
        self.MAX_EDGES = int(((self.NUM_VERTICES) * (self.NUM_VERTICES - 1)) / 2)
    ...
    def init_P0_population(self, pop_size = None):
        ...
        while i < pop_size:
            ...
            model = Model(chromosome, config, self.CELLS[config[self.MAX_EDGES]], self.STACKS[config[self.MAX_EDGES + 1]], self.NBR_FILTERS[config[self.MAX_EDGES + 2]], NUM_CLASSES)
            ...
            if not isSame:
                ...
                self.allModels[model.solNo] = {"org_matrix": model.org_matrix.astype("int8"), 
                                               ...
                                               "matrix": model.matrix.astype("int8"),
                                               "ops": model.ops.astype("int8"),
    ...
    def get_opposite_model(self, model, a = 0, b = 1):
        ...
        opposite_model = Model(opposite_chromosome, config, self.CELLS[config[self.MAX_EDGES]], self.STACKS[config[self.MAX_EDGES + 1]], self.NBR_FILTERS[config[self.MAX_EDGES + 2]], NUM_CLASSES)
        ...
        while counter < len(self.P0):
            ...
            if not isSame:
                ...
                self.allModels[opposite_model.solNo] = {"org_matrix": opposite_model.org_matrix.astype("int8"), 
                                                        ...
                                                        "matrix": opposite_model.matrix.astype("int8"),
                                                        "ops": opposite_model.ops.astype("int8"),
    ...
    def checkSolution(self, model):
        ...
        for m in self.allModels.keys():
            model_2 = self.allModels[m]
            if np.array_equal(model.matrix, model_2["matrix"]) and np.array_equal(model.ops, model_2["ops"]):
                return True, model_2
    ...
    def vector_to_config(self, vector):
        ...
        # Operations
        start_idx = idx + 3
        for idx in range(start_idx, start_idx + max_edges):
            config[idx] = self.get_param_value(vector[idx], len(self.OPS)) + 1 # Sıfır gelmesini engellemek için
    ...
    def evolve_generation(self):
        ...
        for j in range(self.pop_size):
            ...
            model = Model(trial, config, self.CELLS[config[self.MAX_EDGES]], self.STACKS[config[self.MAX_EDGES + 1]], self.NBR_FILTERS[config[self.MAX_EDGES + 2]], NUM_CLASSES)
            self.solNo += 1
            model.solNo = self.solNo
            trials.append(model)
        ...
        for j in range(self.pop_size):
            ...
            if isSameSolution:
                ...
                mutant = Model(sol["chromosome"], cfg, self.CELLS[config[self.MAX_EDGES]], self.STACKS[config[self.MAX_EDGES + 1]], self.NBR_FILTERS[config[self.MAX_EDGES + 2]], NUM_CLASSES)
                ...
                self.allModels[mutant.solNo] = {"org_matrix": mutant.org_matrix.astype("int8"), 
                                                ...
                                                "matrix": mutant.matrix.astype("int8"),
                                                "ops": mutant.ops.astype("int8"),
...
if __name__ == "__main__":
    ...
    rnd_val_indexes = random.sample(range(val_dataset.__len__()), int(val_dataset.__len__() / 2))
